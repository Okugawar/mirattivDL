...
import Ffmpeg.TranscodeFailedException
import Ffmpeg.{Preset, VideoProfile, VideoUtil}
import Ffmpeg.codec.{AudioCodec, VideoCodec}
import Ffmpeg.filter.VideoFilter
...
class FFMPEGTranscoder private(overwrite: Boolean = true, args: List[FFMPEGArgument]) {
  ...
  def i(url: String) = withArgs("-i", FFMPPEGUrl(url))
  ...
  def execute(monitor: Option[TranscodeListener] = None, nicePriority: Option[Int] = None): Unit = {
...
//    val info = VideoUtil.info(inputFile)
    ...
    val niceCommand = nicePriority match {
      ...
      case None => command ::: List(if (overwrite) "-y" else "-n")
    }
...
case class FFMPPEGUrl(url: String) {
